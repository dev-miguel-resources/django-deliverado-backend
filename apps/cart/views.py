from django.shortcuts import render,reverse,redirect
from django.contrib.auth import authenticate, login
from django.utils.crypto import get_random_string
from dapi.settings import base

from apps.directory.models import *

from tbk import WebpayService, Commerce, INTEGRACION, PRODUCCION
from django.views.decorators.csrf import csrf_exempt

 
from datetime import date, timedelta



COMMERCE_CODE = "597020000541"

KEY_DATA = """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----\
"""

CERT_DATA = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----\
"""

TBK_CERT_DATA = """\
-----BEGIN CERTIFICATE-----

JvD7YLhPvCYKry7N6x3l
-----END CERTIFICATE-----\
"""

commerce = Commerce(COMMERCE_CODE, KEY_DATA, CERT_DATA, TBK_CERT_DATA, INTEGRACION)



@csrf_exempt
def payOrder(request,id):
	
	  
	# register transaction in db
	order = Order.objects.get(pk=id)
	
	webpay = WebpayService(commerce)
	amount = 100000
	buy_order = 100000
 
	domain = "https://deliverado.kernelcode.cl/"
	#domain = "http://127.0.0.1:8000/"
	
 
 
 
	return_url = domain + 'cart/return/'
	final_url = domain + 'cart/final/'

	transaction = webpay.init_transaction(amount, buy_order, return_url, final_url)
	

	


	data = { 
	"transaction":transaction,
	 
	}
	return render(request,'step1.html',data)

@csrf_exempt
def returnWepayWebsite(request):
	webpay_normal = WebpayNormal.init_for_commerce(commerce)
	token = request.POST['token_ws']
	transaction = webpay_normal.get_transaction_result(token)
	transaction_detail = transaction.detailOutput[0]
	transaction_detail = {
		'sharesNumber': transaction_detail.sharesNumber,
		'amount': transaction_detail.amount,
		'commerceCode': transaction_detail.commerceCode,
		'buyOrder': transaction_detail.buyOrder,
		'authorizationCode': transaction_detail.authorizationCode,
		'paymentTypeCode': transaction_detail.paymentTypeCode,
		'responseCode': transaction_detail.responseCode,
	}
	data = {
		"transaction":transaction,
		"token":token,
		"transaction_detail":transaction_detail,

	}
	webpay_normal.acknowledge_transaction(token)
	if transaction_detail['responseCode'] == 0:

		transaction_user = Transaction.objects.filter(buy_order=transaction_detail["buyOrder"])[0]
		Transaction.objects.filter(buy_order=transaction_detail["buyOrder"]).update(
			status=1,
			tbk_sharesNumber = transaction_detail["sharesNumber"],
			tbk_amount = transaction_detail["amount"],
			tbk_commerceCode = transaction_detail["commerceCode"],
			tbk_buyOrder = transaction_detail["buyOrder"],
			tbk_authorizationCode = transaction_detail["authorizationCode"],
			tbk_paymentTypeCode = transaction_detail["paymentTypeCode"],
			tbk_responseCode = transaction_detail["responseCode"],
			tbk_token = token,

			)

		UserSuscription.objects.create(
			user = request.user,
			suscription = transaction_user.suscription,
			quota_initial = transaction_user.suscription.quota,
			quota_current = transaction_user.suscription.quota,
		)
		return render(request,'webpayOkWebsite.html',data)
	else:

		Transaction.objects.filter(buy_order=transaction_detail["buyOrder"]).update(status=2)
		return render(request,'webpayErrorWebsite.html',data)


# 4051885600446623

@csrf_exempt
def wepayFinalWebsite(request):
	try:
		token = request.POST['token_ws']
		transaction_detail = Transaction.objects.get(tbk_token=token)
		msg = generate_email("¡La compra de tu plan se ha realizado satisfactoriamente!","Te invitamos a configurar tu pinpass aquí:","http://dev.tuganas.cl/plataforma/opciones/pinpass/actualizar","Configurar pinpass")
		user_email = request.user.email
		send_email_new_plan.delay(msg,user_email)
	except:
		transaction_detail = None

	# orderBuy = request.POST['TBK_ORDEN_COMPRA']

	orderBuy = "asdkasidjadnaodbua"
	
	data = {
		"transaction_detail":transaction_detail,
		"orderBuy":orderBuy,
		 

	}
	return render(request,'webpayFinalWebsite.html',data)



 
 
